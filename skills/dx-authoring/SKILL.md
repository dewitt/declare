...
| `###` key headings            | Identifiers, inside `##` blocks  | Outside any `##` block; inside `## Intent` |
...
| ----------------- | -------- | ------------------------------------------------ | ---------------------------------- |
| `# <system>`      | Yes      | No                                               | Slug-format string in heading body. |
...
The `#` level-1 heading body is the system identifier. Slug-format
(conventionally kebab-case, no leading numbers). Treat as the
namespace for this declaration.
...
- `**Primary:**` is REQUIRED. Keep it on one line if you can; for
  longer rationale, just continue the paragraph onto the next line.
- `**Secondary:**` is OPTIONAL. The body is an unordered CommonMark
  list (`-` items). Each entry is a goal, not a task. "Be fast" is
  fine; "use a thread pool" is not — that's implementation, and
  belongs nowhere in a declaration.

### 3c. `## Invariants`

A block of `###` key sections. Each key body is the invariant
identifier (a category-prefixed slug); the content under the
heading is the invariant body.
...
Multi-paragraph bodies are fine; they read as ordinary CommonMark:
...
Conventional prefixes: `iface_`, `perf_`, `sec_`, `obs_`
(observability), `data_`, `ux_`. Invent new ones as needed but stay
consistent within a single declaration.

**Each invariant is a black-box statement.** It describes observable
system behavior, never internal architecture. "Uses a Bloom filter"
is not an invariant; "membership queries return false-negative rate
of 0" is.
...
not pin the shape, and a list is the natural fit for an enumeration
of goals.
```

An assumption is a heuristic choice the agent had to make because
the human's intent + invariants did not uniquely determine the
answer. Every assumption entry must include **what was decided** and
**why**. The architect later promotes (move to `## Invariants`) or
rejects (rewrite the code and delete the entry) each one.

An empty `## Assumptions` block (heading with zero `###` children)
is a meaningful state: it asserts "I made no unrecorded heuristic
choices." Use it deliberately.
...
- One contract = one observable outcome. If you need conjunctions,
  prefer multiple contracts.

### 3f. `## Unconstrained`

A block of `###` key sections; each key body is the category name,
and the content under the heading is the description of the
freedom granted.
...
Whenever you (the agent) face a choice not specified by `## Intent`
or `## Invariants`:
...
2. **Pick a stable ID.** Convention: `<file_or_module>.<short_phrase>`,
   e.g. `cli.default_output_format`. Reuse the ID forever; never
   rename.
3. **Write the entry** in `## Assumptions` as a `### <id>` section.
   The body must answer: *what did you decide* and *why was that
   the most defensible choice given the ambiguity*.
4. **Then proceed** with implementation.

Anti-pattern to avoid: appending the assumption *after* the
implementation is written. The whole point is that the assumption
is recorded **before** the heuristic leaks into code.
...
If any answer suggests the constraint isn't truly required, move it
to `## Unconstrained` (with a description) or omit it entirely.
...
- [ ] Every `###` key sits inside a recognized `##` block (and not
      inside `## Intent`).
- [ ] `## Intent` has a `**Primary:**` paragraph.
...
- [ ] Anything the human didn't constrain lives in `## Unconstrained`
      or not at all.
